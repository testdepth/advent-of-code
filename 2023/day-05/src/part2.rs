...
use std::ops::Range;
use nom::{self, IResult, bytes::complete::tag, multi::fold_many1, sequence::{terminated, tuple}, character::complete::{space0, self, multispace0, multispace1, not_line_ending, space1, }};
...
fn seed_range(input: &str) -> IResult<&str, Range<u64>> {
    let (input, (start, _, range, _)) = tuple((complete::u64, space1, complete::u64, space0))(input)?;

    Ok((input, Range {start: start, end: start+range}))
}
...
pub fn process(
    ...
    let seeds = seed_ranges.iter().fold(Vec::new(), 
        | mut acc, range | {
    ...
    let result: Vec<u64> = seeds.into_par_iter().map(
        |num| {
