...
impl Race {
    fn get_range_of_good_runs(&self) -> u32 {
        // upper
        let upper: f32 = (-(self.time as f32)
            - f32::sqrt(f32::powf(self.time as f32, 2.0) - 4.0 * self.distance as f32))
            / (-2.0);

        let lower: f32 = (-(self.time as f32)
            + f32::sqrt(f32::powf(self.time as f32, 2.0) - 4.0 * self.distance as f32))
            / (-2.0);
...
fn parse_vec_nums(input: &str) -> IResult<&str, Vec<u32>> {
    fold_many1(
        terminated(complete::u32, multispace0),
        Vec::new,
        |mut acc: Vec<_>, item| {
            acc.push(item);
            acc
...
fn parse_input(input: &str) -> IResult<&str, Vec<Race>> {
    ...
    Ok((
        input,
        zip(times, distances).into_iter().fold(
            Vec::new(),
...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    ...
    let result: u32 = races
        .iter()
        .map(|race| race.get_range_of_good_runs())
        .product();
    Ok(result.to_string())
}
