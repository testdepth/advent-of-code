...
use nom::{self, sequence::{terminated, tuple}, bytes::complete::tag, character::complete::{multispace0, multispace1,digit1}, multi::{separated_list1, separated_list0}, IResult};
...
impl Race {
    fn get_range_of_good_runs(&self) -> u64 {
        // upper
        let upper: f64 = (-(self.time as f64) - f64::sqrt(f64::powf(self.time as f64, 2.0) - 4.0 * self.distance as f64))/(-2.0);

        let lower: f64 = (-(self.time as f64) + f64::sqrt(f64::powf(self.time as f64, 2.0) - 4.0 * self.distance as f64))/(-2.0);
...
fn parse_input(input: & str) -> IResult<&str,Race> {
    ...
    Ok((input, Race{ time: times.join("").parse::<u64>().unwrap(), distance: distances.join("").parse::<u64>().unwrap() }))
