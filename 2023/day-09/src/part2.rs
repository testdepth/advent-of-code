...
fn calculate_final(numlist: Vec<i64>) -> i64 {
    ...
    calcs.iter().rev().fold(
        0, | mut acc:i64, nums | {
            acc = nums.first().expect("should exist") - acc;
            acc
        })
}
...
mod tests {
    ...
    fn test_calcs() -> miette::Result<()>{
        ...
        assert_eq!(-3, calculate_final(input));
        input = vec![1, 3, 6, 10, 15, 21];
        assert_eq!(0, calculate_final(input));
        input = vec![10, 13, 16, 21, 30, 45];
        assert_eq!(5, calculate_final(input));
        Ok(())
    }
